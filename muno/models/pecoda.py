...
class PeCODANO(nn.Module):
    ...
    out_channels : int
        ...
        Example: For a input with 3 variables (channels) and out_channels=2, the output will have 6 channels (3 variables × 2 codimension).
    ...
    def __init__(
        ...
        assert (
            per_layer_scaling_factors is None or len(per_layer_scaling_factors) == n_layers# + n_layers_fno
        ...
        extended_variable_codimemsion = (
            in_channels + positional_encoding_dim
        )
        if not self.lifting:
    ...
    def _extend_variables(self, x, input_variable_ids):
        """
    ...
    def forward(self, x: torch.Tensor, input_variable_ids=None):
        ...
        x = self._extend_variables(x, input_variable_ids)

        # input variables are lifted to a higher-dimensional space
        if self.lifting:
            x = x.reshape(
                batch, self.extended_variable_codimemsion, *spatial_shape #  * num_inp_var
            )
            x = self.lifting(x)

        x = x.reshape(
            batch, self.hidden_variable_codimension, *spatial_shape # num_inp_var * 
        )
        ...
        for layer_idx in range(1): # self.n_layers
            ...
            if (self.horizontal_skips_map is not None
                ...
                x = x.reshape(
                    batch,
                    self.hidden_variable_codimension, # num_inp_var * 
                    *x.shape[-self.n_dim :],
                )
        ...
        for layer_idx in range(self.n_layers - 1):
            # print(f'layer_idx {layer_idx}, x.shape is {x.shape}') # with output shape of {x[layer_idx]},   {self.fno_layers}
