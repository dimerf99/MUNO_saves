...
class PiecewiseLinearFilterBasis(FilterBasis):
    ...
    def __init__(
        ...
        if len(kernel_shape) == 1:
            kernel_shape = [kernel_shape] + [1,] * (ndim - 1)
        if len(kernel_shape) != ndim or len(kernel_shape) > 2:
            raise ValueError(f"expected kernel_shape to be a list or tuple of length {ndim} but got {kernel_shape} instead.")
...
class MorletFilterBasis(FilterBasis):
    ...
    def __init__(
        ...
        super().__init__(kernel_shape = kernel_shape, ndim = ndim)

    @property
    def kernel_size(self):
        return functools.reduce(lambda x, y: x*y, self.kernel_shape) # self.kernel_shape[0] * self.kernel_shape[1]
    ...
    def compute_support_vals(self, r: torch.Tensor, phi: torch.Tensor, r_cutoff: float, width: float = 1.0):
        ...
        if self._ndim == 2:
            ...
            nkernel = ikernel % (self.kernel_shape[1] * self.kernel_shape[0]) % self.kernel_shape[1]
            mkernel = ikernel % (self.kernel_shape[1] * self.kernel_shape[0]) // self.kernel_shape[1]
            pkernel = ikernel // (self.kernel_shape[1] * self.kernel_shape[0])
