...
from muno.layers.discrete_continuous_convolution import EquidistantDiscreteContinuousConv2d, \
                                                            EquidistantDiscreteContinuousConv2dPlusT
...
class LocalNOBlocks(nn.Module):
    ...
    def __init__(
        ...
        assert (len(n_modes) == len(default_in_shape) or len(n_modes) == len(default_in_shape)+1), "Spatiotemporal dimensions must be consistent"
        ...
        self.domain_length = [2, 2] if domain_length is None else domain_length #  * self.n_dim  
        ...
        # print(f'Initializing localconvs for {self.in_channels} channels & {self.default_in_shape} shape')
        # print(f'DISCO kernel shapes are {self.disco_kernel_shape}')

        if self.n_dim == 2:
            conv = EquidistantDiscreteContinuousConv2d
        elif self.n_dim == 3:
            conv = EquidistantDiscreteContinuousConv2dPlusT
        else:
            raise ValueError(f'Incorrect number of dimenstions: expected 2-d or 3-d data, got {self.n_dim}-d')

        # print(' disco_kernel_shape: ', self.disco_kernel_shape, ' default_in_shape: ', self.default_in_shape)
    ...
    def forward_with_postactivation(self, x, index=0, output_shape=None):
        ...
        if self.disco_idx_list[index] != -1:
            ...
            # print(f'Intermediate shape of localconv is {x_localconv.shape}')
            x_localconv = self.convs[index].transform(x_localconv, output_shape=output_shape)
        else:
            x_localconv = 0

        # print(f'x_local_no {x_local_no.shape}, x_differential {x_differential.shape}, x_localconv {x_localconv.shape}')
        x_local_no_diff_disco = x_local_no + x_differential.reshape(x_local_no.shape) + x_localconv.reshape(x_local_no.shape)
