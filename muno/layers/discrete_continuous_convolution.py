...
def _normalize_convolution_filter_matrix(
    ...
    if dim == 2:
        ...
        if len(kernel_shape) == 1:
            ...
            # kernel_size = (kernel_shape[0] // 3) * kernel_shape[1] + kernel_shape[0] % 2
            raise NotImplementedError('3-dim convolution filters are supposed to be isotropic.')


    # print(f'initializing _normalize_convolution_filter_matrix with {grid_in.shape} -> {grid_out.shape}, and ks = {kernel_size}')
...
def _precompute_convolution_filter_matrix_3d(
    ...
    # check that input arrays are valid point clouds in 2D
    # print(len(grid_in), len(grid_out))
    assert len(grid_in) == 3, "grid_in must be a 2d tensor."
    assert len(grid_out) == 3, "grid_out must be a 2d tensor."
    assert grid_in.shape[0] == 3, "grid_in first element must match problem dim.: 3d tensor."
    assert grid_out.shape[0] == 3, "grid_out first element must match problem dim.: 3d tensor."

    n_in = grid_in.shape[-1]
    n_out = grid_out.shape[-1]

    grid_in = grid_in.reshape(3, 1, n_in)
    grid_out = grid_out.reshape(3, n_out, 1)
...
class DiscreteContinuousConv(nn.Module, metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        if isinstance(kernel_shape, int):
            self.kernel_shape = [kernel_shape,] * ndim
        else:
        ...
        # print(f'DISCO shape is: {self.weight.shape}, from: self.groupsize {out_channels}, {self.groupsize}, {self.kernel_size}')
...
class EquidistantDiscreteContinuousConv2d(DiscreteContinuousConv):
    ...
    def __init__(
        ...
        # print(2*radius_cutoff * in_shape[0], self.domain_length[0], 2*radius_cutoff * in_shape[1], self.domain_length[1])
        # print(f'In init: self.psi_local_h {self.psi_local_h} & self.psi_local_w {self.psi_local_w}')
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        h_pad = (kernel.shape[-2] - 1) // 2 #(self.psi_local_h + 1) // 2 - 1
        w_pad = (kernel.shape[-1] - 1) // 2 #(self.psi_local_w + 1) // 2 - 1
...
class EquidistantDiscreteContinuousConv2dPlusT(EquidistantDiscreteContinuousConv2d):
    """Discrete-continuous convolutions (DISCO) on equidistant 2d spatial grids + time.
    It is implemented as described in [1]_. This implementation maps to 2d convolution
    kernels which makes it more efficient than the unstructured implementation
    above. Due to the mapping to an equidistant grid, the domain lengths need
    to be specified in order to compute the effective resolution and the
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        h_pad = (kernel.shape[-2] - 1) // 2 #(self.psi_local_h + 1) // 2 - 1
        w_pad = (kernel.shape[-1] - 1) // 2 #(self.psi_local_w + 1) // 2 - 1
        h_pad_extra = kernel.shape[-2] - 1 - 2 * h_pad  # 0 for odd, 1 for even
        w_pad_extra = kernel.shape[-1] - 1 - 2 * w_pad        

        x = nn.functional.pad(self.q_weight * x, 
                              (w_pad, w_pad + w_pad_extra,
                               h_pad, h_pad + h_pad_extra))
        ...
        out = einops.rearrange(out, '(b t) c ... -> b c t ...', b = B, c = C, t = T)
        # print(f'After conv layers: out.shape is {out.shape}')
