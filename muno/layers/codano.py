...
from muno.layers.transformer import AttentionBlock
...
class CODALayer(nn.Module):
    ...
    def __init__(
        ...
        **_kwargs,
        ...
        attn_enc = AttentionBlock(query_dim     = self.token_codimension,
                                  context_dim   = self.token_codimension,
                                  heads         = self.n_heads, 
                                  dim_head      = self.head_codimension,
                                  to_q_model    = FNOBlocks,
                                  to_q_params   = {'heads_dim': 1, 'n_heads': self.n_heads},
                                  to_kv_model   = FNOBlocks,
                                  to_kv_params  = merge_dicts({'resolution_scaling_factor': 1 * scale,
                                                              'conv_module': conv_module},
                                                              kqv_args,
                                                              shared_fno_configs),
                                  to_out_model  = nn.Identity, # nn.Linear, #nn.Identity,
                                  to_out_params = {'in_features': int(kqv_args['out_channels']),
                                                  'out_features': int(kqv_args['out_channels'])}, #  self._latent_queries_features}, or self.token_codimension
                                  channel_wise  = False)        
